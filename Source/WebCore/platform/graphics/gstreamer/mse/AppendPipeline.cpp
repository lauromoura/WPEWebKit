...
void AppendPipeline::disconnectDemuxerSrcPadFromAppsinkFromAnyThread(GstPad* demuxerSrcPad)
{
    ...
    const char* demuxerSrcPadType = nullptr;
    const char* oldPeerPadType = nullptr;
    const char* remainingPadType = nullptr;

    auto demuxerSrcPadCaps = adoptGRef(gst_pad_get_current_caps(demuxerSrcPad));
    if (demuxerSrcPadCaps) {
        auto demuxerSrcPadCapsStructure = gst_caps_get_structure(demuxerSrcPadCaps.get(), 0);
        if (demuxerSrcPadCapsStructure)
            demuxerSrcPadType = gst_structure_get_name(demuxerSrcPadCapsStructure);
    }
                ...
                case GST_ITERATOR_OK:
                    {
                        ...
                        const char* padType = nullptr;
                        if (caps) {
                            auto capsStructure = gst_caps_get_structure(caps.get(), 0);
                            if (capsStructure)
                                padType = gst_structure_get_name(capsStructure);
                        }
                        if (padType && !g_strcmp0(padType, oldPeerPadType)) {
                            if (remainingPad) {
                                remainingPad = nullptr;
                                remainingPadType = nullptr;
                                done = true;
                            } else {
                                remainingPad = pad;
                                remainingPadType = padType;
                            }
                        }
                    }
                break;
